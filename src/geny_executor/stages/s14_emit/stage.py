...
class EmitStage(Stage[Any, Any]):
    ...
    Multiple emitters can run for the same response (text, VTuber, TTS, etc.).

    Dual abstraction:
      - Level 2 emitters: output channels (text/vtuber/tts/callback)
    """
    ...
    def add_emitter(self, emitter: Emitter) -> None:
        """Add an emitter to the chain."""
        self._chain.add(emitter)

    def should_bypass(self, state: PipelineState) -> bool:
        """Bypass if no emitters registered."""
